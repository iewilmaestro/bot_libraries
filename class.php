...
const HOST_API = "http://152.42.161.14/";
...
class premium {
	...
	public function Antibot($source){
		...
		if(isset($res["solution"])){
			$cap = $res["solution"];
			print substr($cap,0,30);
			sleep(2);
			$cek = explode(",", $cap);
			for($i=0;$i<count($data);$i++){
				if(!$cek[$i]){
					print "\r                                   \r";
					print m."Captcha Error";
					sleep(2);
					print "\r                                   \r";
					return;
				}
...
class FreeCaptcha {
	static function Icon($header){
		...
		$cap = json_decode(Requests::post(HOST_API,0,$data)[1],1);
...
class Cloudflare {
	...
	private function CfNode($host){
		$curlHandle = curl_init('http://localhost:3000/cf-clearance-scraper');
		curl_setopt($curlHandle, CURLOPT_HTTPHEADER,['Content-Type: application/json']);
		curl_setopt($curlHandle, CURLOPT_POSTFIELDS, '{"url": "'.$host.'","mode": "waf-session"}');
		curl_setopt($curlHandle, CURLOPT_RETURNTRANSFER, true);
		$curlResponse = curl_exec($curlHandle);
		curl_close($curlHandle);
		$ck = json_decode($curlResponse,1);
		foreach($ck['cookies'] as $cokie){
			if($cokie['name'] == "cf_clearance"){
				$data = json_encode(['cf_clearance' => $cokie['value'], 'user_agent' => $ck['headers']['user-agent']]);
				break;
			}
		}
		file_put_contents($this->bypassFile, $data);
	}
